...
class TestSettings:
    ...
    def test_settings_none_when_not_set(self):
        ...
        with patch.dict(os.environ, {var: '' for var in env_vars_to_clear}, clear=False):
            ...
            assert config.Settings.JIRA_SERVER == '' or config.Settings.JIRA_SERVER is None
            assert config.Settings.JIRA_USER == '' or config.Settings.JIRA_USER is None  
            assert config.Settings.JIRA_API_TOKEN == '' or config.Settings.JIRA_API_TOKEN is None
