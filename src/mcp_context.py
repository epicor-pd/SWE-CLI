...
def build_context_instructions(ado_org: str, ado_project: str, ado_repos: list[str]) -> str:
    logger.info("Building MCP context instructions for org=%s project=%s repos=%s",
                ado_org, ado_project, ",".join(ado_repos))
    repo_lines = []
    for r in ado_repos:
        repo_lines.append(f"""
