...
def estimate_prompt_tokens(jira_content: str, context_instructions: str, prompt_template: str) -> int:
    """
...
def truncate_large_strings(data: Any, max_length: int = MAX_MCP_STRING_LENGTH) -> Any:
    ...
    if isinstance(data, str):
        if len(data) > max_length:
            ...
            truncation_msg = f"\n\n[TRUNCATED: Original length was {len(data)} characters, showing first {truncated_length} characters]"
            return data[:truncated_length] + truncation_msg
        return data
    
    elif isinstance(data, dict):
        return {key: truncate_large_strings(value, max_length) for key, value in data.items()}
...
def summarize_large_content(content: str, max_length: int = MAX_MCP_STRING_LENGTH) -> str:
    """
    ...
    if len(lines) > 100:
        ...
        summary = (
            f"{first_portion}\n\n"
            f"[CONTENT SUMMARIZED: {len(lines)} total lines, {len(content)} characters]\n"
            f"[Showing first {max_lines_per_section} and last {max_lines_per_section} lines]\n\n"
            f"{last_portion}"
        )
    ...
    summary = (
        ...
        f"[CONTENT TRUNCATED: Original had {len(content)} characters ({len(lines)} lines)]\n"
        f"[Content too large for MCP transmission - showing first {truncated_length} characters]"
    )
...
def get_content_size_info(content: str) -> Dict[str, Union[int, str]]:
    ...
    return {
        ...
        'recommended_action': 'truncate' if len(content) > MAX_MCP_STRING_LENGTH else 'none'
    }
...
def log_large_content_warning(content: str, source: str = "unknown") -> None:
    ...
    if info['exceeds_limit']:
        logger.warning(
            ...
            source, info['character_count'], info['line_count'], info['size_mb']
        )
